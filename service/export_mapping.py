#!/usr/bin/env python3
"""Export a run as the mapping CSV the viewer reads.
...
`standard_outcome`, and this join puts it back on every row. Nothing is lost
...
def sort_key(row):
    ...
    return (str(row["semester"]), int(row["unit"]) if str(row["unit"]).isdigit() else 999,
            0 if token.isdigit() else 1,
            int(token) if token.isdigit() else 0, token, row["standard_id"])


def main(argv=None):
    ap = argparse.ArgumentParser(description="Export a run as the mapping CSV.")
    ap.add_argument("--run", type=int, required=True)
    ap.add_argument("--course-code", required=True, help="e.g. AIF")
    ap.add_argument("--semester", default="", help="S1 / S2, or blank")
    ...
    with psycopg.connect(DSN, row_factory=dict_row) as conn:
        run = conn.execute("""
            SELECT r.*, ss.framework, ss.standard_set, ss.set_type,
                   ss.framework_year
        ...
        rows = conn.execute(f"""
            SELECT s.identifier, ar.evidence, ar.note, ar.level,
                   ...
                   cu.displayed_number, u.script_name, l.lesson_group_name
            ...
            {"run": args.run, "course": run["course_id"]}).fetchall()
    ...
    for row in rows:
        ...
        if row["is_choice_level"]:
            notes.append(f"optional pathway capped at Introduced"
                         + (f" ({row['choice_option']})" if row["choice_option"] else ""))
        for f in (row["flags"] or []):
            if f.get("kind") == "artifact_mismatch":
                notes.append("artifact caveat")
            if f.get("kind") == "overclaim":
                notes.append("lesson carries many standards")
        if row["lesson_group_name"] and "Alternate" in row["lesson_group_name"]:
        ...
        out.append({
            ...
            "mastery": MASTERY_LABEL.get(row["aggregate"] or row["level"], ""),
    ...
    name = (f"{args.course_code}_{run['framework']}_{run['standard_set']}_"
            f"{run['framework_year']}_mapping.csv")
    ...
    print(f"  {len({(r['unit'], r['lesson']) for r in out})} distinct lessons")
